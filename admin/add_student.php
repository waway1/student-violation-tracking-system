<?php
/* Admin: form + handler to add a new student record. */
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $email      = trim($_POST['email'] ?? '');   // real address = violation emails can reach them
        ...
        if ($u->fetch()) {
            ...
            try {
                ...
                $prov = vts_provision_student_account($conn, $school_id, [
                    ...
                    'course' => $course, 'college_id' => $college_id, 'email' => $email,
                ]);
                ...
                exit();
            } catch (PDOException $e) { error_log('Admin add student failed: ' . $e->getMessage()); $error = 'The student could not be saved. Please check the details and try again.'; }
        }
    }
}
...
<?php echo csrf_field(); ?>
    ...
    <div class="form-grid">
      <div class="field full"><label for="email">Email <span style="font-weight:500;color:var(--text-soft);">(optional — but required to receive violation notices)</span></label>
        <input id="email" type="email" name="email" class="vts-input" placeholder="student@gmail.com"
               value="<?php echo htmlspecialchars($_POST['email'] ?? ''); ?>">
        <small class="field-hint">Without an email the student is still scannable, but can only see violations in-app (no Gmail notice).</small></div>
    </div>
    </fieldset>
    <script>
