<?php
/* Admin: student list with search, CRUD and import/export. */
...
/* ---- Roster actions: import (.xlsx/CSV/pasted rows), delete, unregister, clear all ----
   (Consolidated here from the old standalone admin/roster.php, which is now removed —
   this "Enrolled Students" tab is the one home for roster management.) */
$rosterNotice  = '';
$rosterErrNote = '';
...
try {
  ...
  error_log('Admin students query failed: ' . $e->getMessage());
...
<div class="admin-welcome-row">
  ...
  <div class="u-actions">
    ...
    <!-- One Export button. These were two outline buttons sitting side by
         side whose labels ("Export for Scanner" / "Excel copy") gave no clue
         which one a given task wanted; the menu says what each is for. -->
    ...
    <form method="POST" action="students.php" enctype="multipart/form-data" id="rosterImportForm" style="display:inline-flex;margin:0;">
      <?php echo csrf_field(); ?>
...
<div class="recent-table-wrap table-responsive">
  ...
  <table class="data-table">
    <thead><tr><th>Student ID</th><th>Full Name</th><th>Course</th><th>Year</th><th>Section</th><th>Status</th><th>Action</th></tr></thead>
    <tbody>
...
style="width:34px;height:34px;border-radius:50%;object-fit:cover;flex-shrink:0;border:1.5px solid var(--border-soft);">
        ...
        <td class="u-nowrap">
          <?php /* The eye is for everyone who can open this page; the pen and
                   the bin are Admin's. An OSA who may only look now sees
                   exactly one control, rather than two that answer back. */ ?>
          <a href="view_student.php?id=<?php echo $row['id']; ?>" class="btn-outline btn-sm"
             title="View this student's record"><i class="fas fa-eye"></i></a>
          <?php if (($_SESSION['role'] ?? '') === 'Admin'): ?>
          <a href="edit_student.php?id=<?php echo $row['id']; ?>" class="btn-outline btn-sm" title="Edit Student"><i class="fas fa-pen"></i></a>
          ...
          <?php endif; ?>
          <form method="POST" action="delete_student.php" class="inline-form" onsubmit="return confirm('Delete this student?\n\nTheir violation records go with them and cannot be recovered.');"><?php echo csrf_field(); ?><input type="hidden" name="id" value="<?php echo $row['id']; ?>"><button type="submit" class="btn-danger btn-sm" title="Delete"><i class="fas fa-trash"></i></button></form>
          <?php endif; ?>
        </td>
      </tr>
    <?php endforeach; else: ?>
